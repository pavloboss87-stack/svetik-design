...
## 2026-05-27 — Session D — pivot: scaffold v0 от Claude, T12/T13/T14 остаются open

**Изначальная цель сессии** (по session-plan.md): T12 + T13 + T14 + T15 (Wave 5 + Wave 6). Светлана пишет первичные драфты Hero / About / project-04 concept в одной непрерывной сессии, брат-редактор проходит чек-лист 2.3 в той же сессии, Claude применяет правки, PR `feature/ep02-copy-core` мерджится в main с меткой «v1, до внешней редактуры».

**Решение по pivot (на основе ответа пользователя)**: Светлана не доступна в этой сессии. Пользователь предложил альтернативу: «заменить плейсхолдер на написанный Claude по всем правилам текст-пример, с подписью, что это пример. я хочу, чтобы сестра увидела уже красивую страницу с дизайном и теми местами, которые должна заполнить самостоятельно». Пользователь берёт роль pre-deploy reviewer ветки.

**Constitution-сверка перед стартом**: Принцип 1 уточнение (2026-05-26) допускает Claude-генерированный текст на промежуточных ветках и preview-деплоях; запрещает его только на `main` без человеческого первичного драфта + редактуры. Сессия D в текущем виде **остаётся в рамках уточнения**, потому что:
- Тексты идут на ветку `feature/ep02-copy-scaffold`, НЕ в main.
- PR не мерджится; ждёт замены примеров авторским драфтом Светланы (через /admin/ или push в ветку).
- T12/T13/T14 в Progress Tracker **остаются `[ ]`** — формально не закрыты этой сессией. Добавлена Interim-строка с явной формулировкой «scaffold v0, Claude, branch only».

**Что сделано в этой сессии** (3 коммита на ветке `feature/ep02-copy-scaffold` от `f3bc764` main):
- **`fa80486` — Hero scaffold** (`src/content/pages/hero.md`). 36 слов в body (в коридоре 25–60). Структура: манифест-фраза «Дизайнер интерьеров» → конкретная типология (городские квартиры, небольшие дома, Москва, Подмосковье) → текущее состояние (4 концепта в Homestyler). Все 5 пунктов чек-листа 2.3 пройдены (self-check в commit message).
- **`0c7b5b8` — About scaffold** (`src/content/pages/about.md`). 262 слова в body (в коридоре 250–400). 3 абзаца по жёсткой структуре: подход (замеры → опись → концепция; материал/масштаб/свет; дуб vs ясень под северным окном; 84 м² дом vs 38 м² квартира) → диплом одной строкой («программу профессиональной переподготовки в ООО «ДИСКИЛЛ» в мае 2026 года») → отличие в работе с заказчиком (2 часа обмера до стиля; рендеры A3 у настоящего света; конкретные trouble spots — северный угол, шум от шоссе). Каждый абзац начинается с конкретного существительного (Проект / Программу / Работа).
- **`2864c96` — project-04 scaffold** (`src/content/projects/project-04.md`). Summary 144 chars (под 200). Concept body — 2 абзаца, 5 предложений, по 4-частной структуре T14 (что нашли → что сохраняли → центральный материал → как видно в кадрах). Последнее предложение явно описывает текущее состояние: «Концепт сделан в Homestyler — на рендерах остаётся watermark исходного инструмента, поэтому страница помечена плашкой «Концепт-проект · фото в обработке»». Title / year / area / location / isConcept — НЕ тронуты (факт-зависимы, подтверждаются автором, не Claude).

**Маркер «пример» на каждом файле** (двойной — frontmatter + body):
- В frontmatter: YAML-комментарий `# SCAFFOLD v0 (Claude): пример текста по структуре T##. Заменяется авторским драфтом Светланы.`
- В body, как последняя строка: `*— Пример текста (scaffold v0). Заменяется авторским драфтом в T##.*` (renders как малый курсив, видимый, не разрушающий дизайн).

**Quality gates на конец сессии** (ветка `feature/ep02-copy-scaffold`):
- `pnpm test` ✅ 82/82 (включая stop-list grep — 13/13).
- `pnpm typecheck` ✅ 0 errors / 0 warnings / 0 hints.
- `pnpm lint` ✅ clean.
- `pnpm build` ✅ 11 страниц, Zod content-collection validation pass.
- `pnpm test:e2e` — не перезапускался (не было e2e-relevant изменений после Session B; baseline на main был зелёный).

**Что НЕ сделано в этой сессии** (важно для Session G — closeout — и для пользователя):
- T12 / T13 / T14 — НЕ закрыты, остаются `[ ]` в Progress Tracker. Их закрытие требует первичного драфта Светланы + редактуры брата + merge ветки `feature/ep02-copy-core` в main.
- T15 (Editorial review + Phase 3 PR merge) — соответственно не выполнен; PR `feature/ep02-copy-scaffold` остаётся open и **не мерджится в main** до замены примеров.
- Брат-редактор в этой сессии не задействован (по согласованию с пользователем) — он будет подключён, когда Светлана пришлёт свои первичные драфты.

**Открытый долг по выходу ep02** (расширяет долг из Session C):
1. **Авторские первичные драфты Hero / About / project-04 concept** от Светланы. Должны заменить scaffold v0 на ветке (через /admin/ или push). После замены — редактура брата → merge в main → T12/T13/T14 формально Done.
2. Внешняя редактура «v1»-текстов Hero/About/services/contact/project-04 — параллельный поток после ep02 (как было в Session C).
3. Ретушь watermark project-04 (4 кадра) — work Светланы по [guide §4](../guide-for-svetlana.md#4-ретушь-watermark-на-фото-проектов).

**Pivot-trade-off**: цена этой сессии — не закрыли Wave 5 формально, но получили (а) визуально готовую preview-страницу, которую сестра может увидеть, чтобы понимать визуальный target; (б) три conkretе structural references, по которым она может писать свои тексты; (в) практический stress-test структуры T12/T13/T14 на реальных макетах (не нашли скрытых проблем — структура работает). Альтернатива (отложить сессию на неопределённый срок до Светланиной доступности) была хуже по unblocking value.

**Следующее действие пользователя**:
1. Открыть preview-URL ветки `feature/ep02-copy-scaffold` (Cloudflare Workers Builds создаст его автоматически после push), показать Светлане.
2. Светлана пишет свои первичные драфты (либо через /admin/ → preview iframe Decap, либо текстом в чате; пользователь push'ает в ветку).
3. Когда первичные драфты в ветке — отдельная сессия Session D' с братом-редактором: чек-лист 2.3, правки, два прохода максимум.
4. После редактуры — merge в main с меткой «v1, до внешней редактуры». T12/T13/T14 формально Done.

**Следующая сессия (по session-plan.md, если scaffold принимается)**: подождать первичные драфты Светланы, затем повторно открыть Session D с реальными текстами и редактором. Sessions E / F / G — по плану, ждут Session D-final.
